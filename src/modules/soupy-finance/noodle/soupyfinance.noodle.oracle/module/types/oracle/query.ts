...
const baseQueryPricesRequest: object = { assets: "" };
...
const baseQueryPricesResponse: object = { data: "" };
...
export const QueryPricesResponse = {
  ...
  decode(input: Reader | Uint8Array, length?: number): QueryPricesResponse {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseQueryPricesResponse } as QueryPricesResponse;
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
...
export class QueryClientImpl implements Query {
  ...
  Params(request: QueryParamsRequest): Promise<QueryParamsResponse> {
    ...
    const promise = this.rpc.request(
      "soupyfinance.noodle.oracle.Query",
      "Params",
      data
    );
  ...
  Prices(request: QueryPricesRequest): Promise<QueryPricesResponse> {
    ...
    return promise.then((data) => QueryPricesResponse.decode(new Reader(data)));
  }
}
