...
export var modules: {[key: string]: ModuleClient} = {
	bank: {
		tx: null,
		query: null,
		txGen: bankTxClient,
...
export async function setRestAddr(_restAddr: string) {
...
export async function setRpcAddr(_rpcAddr: string) {
...
export async function setWallet(_wallet: OfflineSigner) {
	if (_wallet == null)
...
export async function setEventsAddr(addr: string) {
	if (!socket)
		socket = new Socket(addr);
}

export function handleEvents(query: string, handler: Function) {
	socket.registerEventsListener(query, handler);
}

export function createOrder(market: string, price: number, quantity: number, type: string="limit", flags: string="") {
